...
export const TeachingManagementPage: React.FC = () => {
  ...
  useEffect(() => {
    console.log("TeachingManagementPage mounted")
  }, [])
  ...
  useEffect(() => {
    const fetchTeachings = async () => {
      try {
        ...
        const response = await api.getModelData("Teachings", {
          page: currentPage,
  ...
  const handleDeleteTeaching = async (id: number) => {
    if (window.confirm("Are you sure you want to delete this teaching assignment?")) {
      try {
        ...
        await api.deleteModel("Teachings", id)
        ...
        <main className="teaching-management-content">
          <div className="teaching-management-actions">
            <div className="search-and-actions">
              ...
              />
              <AddButton text="Add Teaching" onClick={handleAddTeaching} />
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="error-container">
              ...
              <button
                ...
                onClick={() => {
                  console.log("Retry button clicked: resetting currentPage and debouncedSearchTerm")
                  setCurrentPage(1)
                  setDebouncedSearchTerm(searchTerm)
                }}
              >
