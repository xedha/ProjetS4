...
def delete_model(request):
    ...
    try:
        ...
        model_name = data.get("model")                # e.g., "Enseignants"
        ...
        Model = apps.get_model('link_db', model_name)  # replace 'link_db' with your app name
        if not Model:
...
def add_model_row(request):
    ...
    try:
        ...
        Model = apps.get_model('link_db', model_name)  # replace with your app name
        if not Model:
            return JsonResponse({'error': 'Model not found'}, status=404)
